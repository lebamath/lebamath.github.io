...
由于公式 (3) 指数部分的 $$\tau'_n(t)$$是要与很大的 $$f_0$$ 作用，所以不能做近似。 
...
可以看到，$$e^{-j2\pi \tau'_n(0) f'}$$ 这一项在求和公式里面，所以，$$\lvert H(f',t) \rvert$$ 的值就与 $$f'$$ 有关，因此，不同的频率，其衰减系数也不一样，这是频率选择性信道。



如果要想是频率非选择性信道或者叫平坦衰落信道，则需要 $$e^{-j2\pi \tau'_n(0) f'}$$  这个与求和公式无关，即与 n无关。若符号长度 $$T_{sym}$$ 远大于 最大时延，即 $$max\lvert \tau'_n - \tau'_m \rvert<< T_{sym}$$，则不同的路径 n，$$\tau'_n(0)$$ 都取相同的值，令 $$\tau'_0 = \tau'_n(0)$$ ，那么公式 (7) 就可以写成：
...
从公式 (8) 可以看出，对不同的频率$$f'$$, 其衰减系数  $$\lvert H(f',t)\rvert$$ 都是一样的（因为$$e^{-j2\pi\tau'_0 f'}$$模等一. 因此，这种信道就称之为频率非选择性信道或者叫平坦衰落信道.
