...
频域有 64 个频点，我们在第二个频点上发送一个信号，幅度为 1，相位为 $$\pi/4$$，这个信号可以表示为 $$e^{j\pi/4}$$. 按照直观的理解，我们发送的信号是：
...
在这个例子中，由于只有 $$X[1] = e^{j\frac{\pi}{4}}$$ ,其它都是 0，上面公式中的求和项，就退化为只有 k=1参与了运算：
...
傅立叶变换中的   $$\left \{\frac{e^{-j2\pi \frac{k}{N}n}}{\sqrt N},n=0,1,\cdots,N-1 \right \}$$构成一个合格的基向量，而 $$\left \{ e^{-j2\pi \frac{k}{N}n}, n=0,1,\cdots,N-1 \right \}$$不是一个基向量，因为其长度是 $$\sqrt N$$.
